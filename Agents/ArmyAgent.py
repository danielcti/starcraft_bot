...
class BotDaGalera(BotAI):
    ...
    def find_target(self, state):
        # caso a gente saiba onde as unidades inimigas estao atacamos ela
        if len(self.enemy_units) > 0:
            return random.choice(self.enemy_units)
        # caso nao sabemos, mas sabemos onde estao as estruturas inimigas atacmos ela
        elif len(self.enemy_structures) > 0:
            return random.choice(self.enemy_structures)
        # e caso nao sabemos nenhum dos 2 vamos para o starting point do inimigo
        else:
            return self.enemy_start_locations[0]

    async def attack(self):
        # se tivemos mais de 15 unidades vamos atras de qualquer alvo pra atacar
        if self.units(UnitTypeId.MARINE).amount > 15:
            for s in self.units(UnitTypeId.MARINE).idle:
                s.attack(self.find_target(self.state))
        # caso mais de 3 unidades e sabemos onde as unidades inimigas estao atacaremos elas
